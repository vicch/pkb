...
By nature, data is stateful, so there must be a deterministic mechanism to dispatch data requests to the servers that host the data.

Data partitioning mechanism depends on data access pattern. _Data that need to be accessed together should be stored on the same server._
