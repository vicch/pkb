...
#+CAPTION: 字符由 @@html:<code>'</code>@@ 包围时，表示它对应的整数值，即字符码（character code）。
#+BEGIN_SRC cpp
'A' == 65;
#+END_SRC
...
- 实质是字符数组。
- 由 @@html:<code>"</code>@@ 包围。
- 由 null 字符 =\0= 结尾，字符串操作函数通过 null 字符来确定字符串的结尾。
...
当加载的头文件用 @@html:<code>""</code>@@ 包围时，编译器会先在当前目录下查找，之后通常在 C++ 标准库目录下查找。
...
1. [[http://en.cppreference.com/w/][cppreference.com]]
1. [[http://www.cplusplus.com/reference/][cplusplus.com]]
1. [[https://google.github.io/styleguide/cppguide.html][Google C++ Style Guide]]

-----

1. [[http://www.tutorialspoint.com/cplusplus/index.htm][C++ Tutorial]]
1. [[http://www.learncpp.com/][Learn C++]]
